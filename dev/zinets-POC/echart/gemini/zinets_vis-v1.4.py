...
def parse_markdown_to_tree_data(markdown_text):
    ...
    for i in range(1, len(lines)):
        ...
        # Count leading tabs
        leading_tabs = len(original_line) - len(original_line.lstrip('\t'))
        if leading_tabs > 0:
    ...
    for i in range(1, len(lines)):
        ...
        # Skip lines that don't start with - (might be additional info)
        if not original_line.lstrip(' \t').startswith('-') and i > 0:
            continue
