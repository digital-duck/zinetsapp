...
def parse_markdown_to_tree_data(markdown_text):
    ...
    for i in range(1, len(lines)):
        ...
        parts = line.split('（')  # Split by Chinese parentheses
        if len(parts) == 1:
            parts = line.split('(')  # Split by English parentheses
...
def get_character_data_from_gemini(characters, debug_flag=True):
    ...
    try:
        ...
        if debug_flag:
            ...
            file_raw = "gemini_response_raw.txt"
        ...
        if debug_flag:
            ...
            file_cleaned = "gemini_response_cleaned.txt"
            with open(file_cleaned, "a", encoding="utf-8") as f:
                f.write(log_header)
                f.write(cleaned_text)
            print(f"Cleaned Gemini response saved to {file_cleaned}")
...
def generate_html(tree_data, character_data, title="ZiNets Vis"):
    """
...
def process_semantic_network(markdown_text, output_file=FILE_OUTPUT, use_gemini=True):
    """
...
if __name__ == "__main__":
    if False:
        ...
        output_file = process_semantic_network(zi_network, output_file="zinets_vis.html")
    ...
    output_file = process_semantic_network(zi_network, output_file="zi_vis.html")
