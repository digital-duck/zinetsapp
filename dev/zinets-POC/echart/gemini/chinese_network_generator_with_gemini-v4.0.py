...
def get_character_data_from_gemini(characters, debug=True):
    """
...
def parse_js_object(text):
    ...
    This is a more robust parser that can handle the actual format returned by Gemini.
    """
    import re
    import json
    ...
    # Step 2: Handle the JSON conversion in multiple steps
    
    # Step 2.1: First, ensure all property names are quoted
    # This regex looks for unquoted property names at the beginning of a line or after a comma
    js_obj_str = re.sub(r'([{,]\s*)([a-zA-Z0-9_$]+)(\s*:)', r'\1"\2"\3', js_obj_str)
    
    # Step 2.2: Replace single quotes with double quotes for string values
    # We need to be careful not to affect already properly double-quoted strings
    # This can be tricky, so we'll use a more manual approach
    
    # First, find all string literals currently quoted with single quotes
    single_quoted_strings = re.findall(r"'([^'\\]*(?:\\.[^'\\]*)*)'", js_obj_str)
    
    # Replace each single-quoted string with double quotes
    for string in single_quoted_strings:
        # Escape any double quotes inside the string content
        escaped_string = string.replace('"', '\\"')
        js_obj_str = js_obj_str.replace(f"'{string}'", f'"{escaped_string}"', 1)
    
    # Step 2.3: Remove trailing commas before closing braces or brackets
    js_obj_str = re.sub(r',(\s*[}\]])', r'\1', js_obj_str)
    
    # Manual fixes for common issues
    # Replace any remaining single quotes with double quotes (not ideal but necessary)
    ...
    try:
        ...
        # Debugging: Log problematic parts of the string
        error_line = int(str(e).split('line')[1].split()[0])
        error_column = int(str(e).split('column')[1].split()[0])
        
        # Extract several lines around the error
        ...
        for i in range(start_line, end_line):
            line_marker = ">" if i == error_line - 1 else " "
            line_num = i + 1
            print(f"{line_marker} {line_num}: {lines[i]}")
            
            # Mark the exact character where the error occurred
            if i == error_line - 1:
                print(f"  {' ' * (error_column - 1)}^")
        
        # Fall back to a more manual parsing approach for specific JSON structure
        try:
            # Try an alternative approach: process line by line for our specific format
            character_data = {}
            current_char = None
            current_property = None
            
            # Very simplified line-by-line parser for our specific format
            # This won't handle all JS objects but works for characterData
            for line in js_obj_str.split('\n'):
                ...
                # Property entry
                    ...
                    if prop_match:
                        prop, value = prop_match.groups()
                        if current_char:
                            character_data[current_char][prop] = value
